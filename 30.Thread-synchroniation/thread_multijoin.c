...
threadFunc(void *arg)
{
    ...
    s = pthread_mutex_lock(&threadMutex);
		printf("Worker thread [%ld] have lock\n",(long)  pthread_self() );
    if (s != 0)
    ...
    thread[idx].state = TS_TERMINATED;
		...
		printf("Worker thread [%ld] going to release lock\n", (long) pthread_self() );
    s = pthread_mutex_unlock(&threadMutex);
    if (s != 0)
        errExitEN(s, "pthread_mutex_unlock");
		printf("Worker thread [%ld] going to signal on condition\n", (long) pthread_self() );
...
main(int argc, char *argv[])
{
    ...
    while (numLive > 0) {
        ...
        if (s != 0)
            errExitEN(s, "pthread_mutex_lock");
				printf("Main thread [%ld] have lock\n", (long) pthread_self() );

				printf("Main thread [%ld] have waiting on condition\n", (long) pthread_self() );
        while (numUnjoined == 0) {
            s = pthread_cond_wait(&threadDied, &threadMutex);
            if (s != 0)
                errExitEN(s, "pthread_cond_wait");
        }
				printf("Main thread [%ld] have done waiting on condition\n", (long) pthread_self() );
        ...
        for (idx = 0; idx < totThreads; idx++) {
            if (thread[idx].state == TS_TERMINATED) {
				...
				printf("Main thread [%ld] going to release lock\n", (long) pthread_self() );
        s = pthread_mutex_unlock(&threadMutex);
        if (s != 0)
            errExitEN(s, "pthread_mutex_unlock");
    }
