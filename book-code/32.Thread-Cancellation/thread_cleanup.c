...
threadFunc(void *arg)
{
    ...
    const char *msg = "pthread_cond_wait()\n";
    while (glob == 0) {
        write(STDERR_FILENO, msg, strlen(msg)); /* Disabled buffering */
        s = pthread_cond_wait(&cond, &mtx);     /* TODO: A cancellation point */
        /*When a pthread_cond_wait() call is canceled, the mutex is automatically 
          relocked before the cleanup handler is invoked. This means the mutex 
          can be safely unlocked in the cleanup handler.*/
        if (s != 0)
